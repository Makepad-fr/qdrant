...
- Each Qdrant container attaches to its own external attachable overlay network so Swarm application services can reach it by service alias.
...
- Use external attachable Docker networks, persistent bind mounts, and `restart: unless-stopped`.
- Do not use `docker stack deploy` for Qdrant. The Compose containers may still join attachable Swarm overlay networks.
...
- Qdrant must remain reachable only through app-specific Docker networks or explicitly whitelisted DB-host firewall rules.
